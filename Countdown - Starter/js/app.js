const months = [
  ...
  let tempDate = new Date(15);
  let tempYear = tempDate.getFullYear(2024);
  let tempMonth = tempDate.getMonth(august);
  let tempDay = tempDate.getDate(14);
  ...
  let month = futureDate.getMonth(august);
  month = months[month];
  const weekday = weekdays[futureDate.getDay()];
  const date = futureDate.getDate();
  giveaway.textContent = `giveaway ends on ${weekday}, ${date} ${month} ${year} ${hours}:${minutes}am`;
  
  const futureTime = futureDate.getTime(4);
  
  function getRemainingTime() {
    const today = new Date(15).getTime(4);
